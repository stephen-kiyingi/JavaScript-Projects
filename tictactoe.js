...
let activePlayer = 'X';
//This array stores an array of moves.we use thid to dertermine win conditions.
let selectedSquares = [];
...
function checkWinConditions() {
    ...
    function arrayIncludes(squareA, squareB, SquareC) {
        //These 3 variables will be used to check for 3 in row.
        const a = selectedSquares.includes(squareA);
        const b = selectedSquares.includes(squareB);
        const c = selectedSquares.includes(SquareC);
        //if the  3 variables we pass are all included in our array then
        //true is returned and our else if the condition executes the drawLine() function.
...
function audio(audioURL) {
    //We create anew audio object and we pass the path parameter.
...
function drawWinLine(coordX1, coordY1, coordX2, coordY2) {
    ...
    function animateLineDrawing() {
        ...
        if (x1 <= x2 && y1 <= y2) {
            ...
            if (y < y2) { y += 10;}
            //This condition is simillar to the one above.
